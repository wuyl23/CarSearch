...
<body>
    ...
    <tbody>
		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><span><?php echo ($vo["text"]); ?></span></td>
			<!-- <td><?php echo ($vo["status"]); ?></td> -->
      <td>
        <!-- <?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Feedback/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>" class="ajax-get">启用</a><?php endif; ?> -->
          <a class="confirm ajax-get" href="<?php echo U('Feedback/remove?id='.$vo['uid']);?>">删除</a>
                </td>
