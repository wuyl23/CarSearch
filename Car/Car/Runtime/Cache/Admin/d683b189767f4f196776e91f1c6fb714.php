...
<body>
	...
	<div class="main-title">
		<h2>优惠管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>

	<div class="cf">
		<a class="btn" href="<?php echo U('edit');?>">新 增</a>
		  <button class="btn ajax-post confirm" url="<?php echo U('del?id='.$order['id']);?>" target-form="ids">删 除</button>
         <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入优惠名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/Car/admin.php?s=/Fcoupon/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
          ...
          <th>状态</th>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><a title="编辑" href="<?php echo U('edit?id='.$order['id']);?>"><?php echo ($order["title"]); ?></a></td>
					<!-- <td><a title="编辑" href="<?php echo U('edit?id='.$order['id'].'pid='.$pid);?>"><?php echo ($order["title"]); ?></a></td> -->
					<td><?php echo ($order["price"]); ?></td>
					<td><?php echo ($order["lownum"]); ?></td>
					<td><?php echo ($order["code"]); ?></td>
					<td><img src="<?php echo (get_cover($order["icon"],'path')); ?>"  width="150" height="50"/></td>
          <td><?php echo ($order["display"]); ?></td>
					<td><?php echo (time_format($order["starttime"])); ?></td>
					<td><?php echo (time_format($order["endtime"])); ?></td>
