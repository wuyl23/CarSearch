...
<body>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><img src="<?php echo (get_cover($vo["headicon"],'path')); ?>"  width="50" height="50"/></td>
			...
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				...
				<!-- <a href="<?php echo U('AuthManager/group?id='.$vo['id']);?>" class="authorize">授权</a> -->
				        <a title = "编辑" href="<?php echo U('edit?id='.$vo['id']);?>">编辑</a>
                <a href="<?php echo U('User/delete?id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
